...
describe('NostrTun.advertise()', () => {
  test('injects Nostr-Tun-Location header with pubkey/relays/ma', async () => {
    ...
    expect(res.headers.get('Nostr-Tun-Location')).toBe(
  ...
  test('defaults ma= to 300 when advertiseTtl omitted', async () => {
    ...
    expect(res.headers.get('Nostr-Tun-Location')).toContain('ma=300')
  })
